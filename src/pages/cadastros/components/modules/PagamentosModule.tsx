...
export default function ProdutosModule() {
  ...
  const openEdit = (item: Product) => {
    setEditing(item);
    setForm({ name: item.name, description: item.description || '', sku: item.sku || '', image_url: item.image_url || '', bling_id: item.bling_id || '' });
    setShowAdvanced(!!(item.sku || item.bling_id));
  };
  const reset = () => { setEditing(null); setForm({ ...EMPTY }); setShowAdvanced(false); };

  const save = async () => {
    if (!form.name.trim()) return;
    setSaving(true);
    const data = { name: form.name, description: form.description || null, sku: form.sku || null, image_url: form.image_url || null, bling_id: form.bling_id || null, updated_at: new Date().toISOString() };
    if (editing) {
  ...
  const toggle  = async (item: Product) => { await supabase.from('products').update({ is_active: !item.is_active, updated_at: new Date().toISOString() }).eq('id', item.id); await reload(); };
  const del     = async (item: Product) => { await supabase.from('products').delete().eq('id', item.id); await reload(); };
  ...
  return (
    <div className="p-5 space-y-5">
      <div className="bg-gray-50 rounded-xl p-4 space-y-3">
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">{editing ? 'Editando produto' : 'Novo produto'}</p>

        <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="Nome do produto *" className={inp} />
        <textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))}
          placeholder="Descrição (opcional)" rows={2} className={`${inp} resize-none`} />
        <input value={form.image_url} onChange={e => setForm(p => ({ ...p, image_url: e.target.value }))} placeholder="URL da imagem (opcional)" className={inp} />
        ...
        {showAdvanced && (
          <div className="space-y-3 pt-1 border-t border-gray-200">
            <div>
              ...
              <input value={form.sku} onChange={e => setForm(p => ({ ...p, sku: e.target.value }))} placeholder="Código SKU do produto" className={inp} />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1.5">
                ID Bling
                <span className="ml-1.5 text-[10px] text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-md">Integração futura</span>
              </label>
              <input value={form.bling_id} onChange={e => setForm(p => ({ ...p, bling_id: e.target.value }))} placeholder="ID do produto no Bling" className={inp} />
            </div>
          </div>
        )}
      ...
      <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-50">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Produtos ({products.length})</p>
        </div>
        <CadastroList items={products} onToggle={toggle} onEdit={openEdit} onDelete={del} onReorder={reorder}
          ...
          renderBadge={item => (
            item.image_url
              ? <img src={item.image_url} alt={item.name} className="w-9 h-9 rounded-lg object-cover border border-gray-100 flex-shrink-0"
                  onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }} />
              : <div className="w-9 h-9 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0"><i className="ri-gift-line text-sm text-gray-400"></i></div>
          )}
          renderExtra={item => (
            <div className="flex items-center gap-2 mt-0.5">
              {item.sku && <span className="text-[10px] text-gray-400">SKU: {item.sku}</span>}
              {item.bling_id && <span className="text-[10px] text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-md">Bling</span>}
            </div>
          )} />
      </div>
    </div>
